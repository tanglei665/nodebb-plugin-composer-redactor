		...
		inline: function()
		{
			return {
				...
				formatUncollapsed: function(tag, params ,newPlugin)
				{
                    ...
                    this.inline.setNodesStriked(nodes, tag, params ,newPlugin);
                ...
                isConvertableAttr: function(node, name, value)
                {
					...
					if (nodeAttrValue)
					{
						...
						// console.log($(node).eq(0), '$(node).nodeName')
						// console.log($(node), '$(node).nodeName')
						// console.log($(node).eq(0).prop("tagName"), '$(node).nodeName')
						// if($(node).eq(0).prop("tagName") === 'P'){
						// 	if($(node).find('span').length === 1){
						// 		$(node).find('span').not('.redactor-selection-marker').css(value);
						// 	}
						// }else{
						...
						if($('#firstSpan').length === 1){
							$('#firstSpan').css(value).removeAttr("id");
						}
					}
                    return 0;
                },
    			isConvertable: function(node, nodeTag, tag, params, newPlugin)
    			{
                    if(newPlugin){
						if (params.data)
                        {
                            ...
                            {
                                count += this.inline.isConvertableAttr(node, 'style', params.style, true);
                            }

                            if (count === Object.keys(params.style).length)
                            {
                                return true;
                            }	
						}
				...
				{
                    var tags = ['b', 'bold', 'i', 'italic', 'underline', 'strikethrough', 'deleted', 'superscript', 'subscript', 'span'];
					var replaced = ['strong', 'strong', 'em', 'em', 'u', 'del', 'del', 'sup', 'sub', 'span'];
				...
				getParams: function(attr, value, type)
				{
 				    var data = false;
					var func = 'toggle';
					if(attr === 'style' && typeof value === 'object') return  { 'func': func, 'style': value } 
...
>>>>>>> 1fd887142e312e73108ea7c2ea02a3a6137df075
		...
		marker: function()
		{
			return {
				...
				insert: function(newPlugin)
				{
					var sel = this.selection.get();
					var range = this.selection.range(sel);
					if(newPlugin){
						this.marker.insertNode(range, this.marker.get(1), true, newPlugin);
				...
				// private
				insertNode: function(range, node, collapse, newPlugin)
				{
					var parent = this.selection.parent();
		...
		selection: function()
		{
			return {
				...
				save: function(newPlugin)
				{
 					this.marker.insert(newPlugin);
			...
			return {
				...
				setDropdownPosition: function(position)
				{
					...
					$('.redactor-dropdown-' + this.uuid).each(function()
					{
						...
						var top = (position === 'fixed') ? self.opts.toolbarFixedTopOffset : $button.offset() && $button.offset().top;
